...
EPG matching is enabled **per Event Group** (there is no global switch); the settings below are global tuning that applies to every group that opts in. Has no effect unless the connected Dispatcharr exposes the program-search API.
...
| **Process each Dispatcharr channel as** | (Shown when the above is on.) How candidates are drawn from each curated channel. **Streams** (default) matches every stream curated inside a channel individually — a channel's streams can fan out across different event channels. **Channels** treats each channel as a unit: Teamarr matches a single representative stream per channel, so the channel's built-in stream lineup is matched as a whole and never fragmented. |
...
Enable it **per Event Group** (Event Group settings → *EPG program matching*) — only groups that opt in are scanned. The channel still exists for its normal lifecycle (filler + upcoming guide); only the linear *stream* swaps in and out near game time.
