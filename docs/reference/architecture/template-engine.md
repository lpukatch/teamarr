...
The template engine resolves `{variable}` placeholders in EPG titles, descriptions, and filler content. It supports 224 variables across 19 categories, 23 condition evaluators, suffix rules for multi-game context, and template-type scoping for the variable picker.

## Architecture

```
TemplateResolver
  ├── VariableRegistry (224 variables, 19 categories)
...
| `templates/variables/` | 19 category modules with 224 variable definitions |
